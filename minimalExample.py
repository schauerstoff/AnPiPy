...
# idee: entferne aus der liste sobald einmal copy gedrückt wurde
# idee show wie viele noch left sind in der liste
import sys
...
class Ui_MainWindow(object):
    ...
    def load_word(self, pos):
        # self.japaneseLab.setText("MainWindow", self.data.japanese[pos])
        # self.englishLab.setText("MainWindow", self.data.english[pos])
        self.japaneseLab.setText(self.data.japanese[pos])
        self.englishLab.setText(self.data.english[pos])
        if(self.data.lang == Language.JAP):
            word = self.data.japanese[pos]
            self.chrome.download_images(word)
            self.fill_images()
        elif(self.data.lang == Language.ENG):
            word = self.data.english[pos]
            self.chrome.download_images(word)
            self.fill_images()
        else:
            word = self.data.english[pos] + " stock photo"
            self.chrome.download_images(word)
            self.fill_images()
    ...
    def change_e(self):
        self.data.lang = Language.ENG
        self.load_word(self.data.pos)

    def change_s(self):
        self.data.lang = Language.STK
        self.load_word(self.data.pos)
    ...
    def setupUi(self, MainWindow):
        ...
        #Eng, Stock
        self.search_button_E.clicked.connect(lambda: self.change_e())
        self.search_button_S.clicked.connect(lambda: self.change_s())
...
if __name__ == "__main__":
    ...
    # timer = QTimer()
    # timer.timeout.connect(ui.fill_images)
    # timer.start(100)  # repeat self.update_labelTime every 1 sec
